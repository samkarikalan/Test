...
function openModeSwitcher() {
  ...
  // Show guide on first time (after login)
  mlShowGuideFirstTime();
...
function updateModePill(mode) {
  ...
  if (mode === 'viewer') {
    ...
    subtitle = (player && (player.displayName || player.name || player.nickname)) || '';
...
function initModeOnLoad() {
  ...
  loadHomeStyle();
  // Update mode pill and appMode to reflect saved mode
...
async function initAppFlow() {
  ...
  const orgVerified   = sessionStorage.getItem('scs_organiser_verified') === '1' ||
                        localStorage.getItem('scs_organiser_verified')   === '1';
  if (savedMode === 'organiser' && orgVerified) {
    applyMode('organiser');
    updateModePill('organiser');
    appMode = 'organiser';
  ...
  _refreshWelcomeSubtitle();
  if (typeof mlSyncLangDisplay === 'function') mlSyncLangDisplay();
  // Show getting started guide on first visit
  mlShowGuideFirstTime();
}
  ...
  overlay.innerHTML = `
    <div class="club-setup-sheet" id="clubSetupSheet">
      ...
      <p style="font-size:0.84rem;color:var(--text-dim);margin-bottom:16px;line-height:1.5">
        You need to be connected to a club. Join an existing club${targetMode === 'organiser' ? '. To create a new club, switch to Vault mode.' : ', or create a club under Vault.'}
      </p>
      ...
      <div id="clubSetupPanelJoin" style="margin-top:14px">
        <input type="text" id="csJoinSearch" class="auth-input" placeholder="🔍 Search Club..." style="margin-bottom:6px" oninput="_clubSetupSearch(this.value)">
        ...
        <input type="password" id="csJoinPassword" class="auth-input" placeholder="${targetMode === 'vault' ? 'Club Admin password' : 'Club Organiser password'}" style="margin-bottom:10px">
  ...
  overlay.innerHTML = `
    <div class="shuttle-sheet" id="shuttleSheet">
      ...
      <button onclick="confirmEndSession()"
        style="width:100%;padding:14px;background:#e63757;color:#fff;border:none;border-radius:14px;font-size:1rem;font-weight:800;cursor:pointer;font-family:inherit;margin-top:12px;">
        ⏹ End Session
      </button>
      <button onclick="skipShuttleAndEnd()"
        style="width:100%;padding:10px;background:none;border:none;color:var(--muted,#888);font-size:0.85rem;cursor:pointer;font-family:inherit;margin-top:6px;">
        Skip — end without recording cost
      </button>
    </div>`;
