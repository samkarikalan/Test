...
function applyTheme(mode) {
  ...
  // Sync all scs-topbar theme buttons
...
function addPlayerokd() {
  ...
  for (let line of lines) {
    ...
    line = line.replace(/\.$/, '').trim(); // Remove trailing dot
...
function deletePlayer(i) {
  ...
  // 4️⃣ Release is_playing in DB — same as playerPlayingRelease() in settings.js
  try {
...
async function goToRounds() {
  ...
  schedulerState.courts = numCourtsInput; // keep alias in sync
  const totalPlayers = schedulerState.activeplayers.length;
  if (!totalPlayers) {
...
async function toggleRound() {
  ...
  if (currentState === "idle") {
    ...
    textEl.textContent = currentRoundIndex === 0 ? (t("startGame") || "Start") : (t("playRound") || "Play Round");
    icon.textContent = " ▶";
...
function showRound(index) {
  ...
  // ✅ Update button label based on round index
  const _btnEl   = document.getElementById("nextBtn");
  const _textEl  = document.getElementById("btnText");
  const _iconEl  = _btnEl ? _btnEl.querySelector(".icon") : null;
  if (_textEl && _iconEl && currentState !== "active") {
    _textEl.removeAttribute("data-i18n");
    _textEl.textContent = index === 0 ? (t("startGame") || "Start") : (t("playRound") || "Play Round");
    _iconEl.textContent = " ▶";
  }
...
function makeRestButton(player, data, index) {
  ...
  ratingBadge.setAttribute('data-player', baseName);
  ratingBadge.textContent = (typeof getActiveRating === 'function')
    ? getActiveRating(baseName).toFixed(1)
    : '—';
  ...
  if (IS_MIXED_SESSION && player?.gender) {
    // Gender-based border + subtle tinted background
    const isFemale = player.gender === 'Female';
    const borderColor = isFemale ? 'rgba(255,101,132,0.8)' : 'rgba(108,99,255,0.8)';
    const bgColor     = isFemale ? 'rgba(255,101,132,0.15)' : 'rgba(108,99,255,0.15)';
    btn.style.border = `2px solid ${borderColor}`;
    btn.style.backgroundColor = bgColor;
    btn.style.color = 'var(--text, #fff)';
  } else {
    // Original rest-count coloring (no gender info)
    if (restMatch) {
...
function loadHelp(sectionKey) {
  ...
  if (!helpData || loadedLang !== lang) {
    fetch(`https://scs-app.com/help_${lang}.json?v=${Date.now()}`)
      .then(res => {
