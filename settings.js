...
function applyTheme(mode) {
  ...
  // Sync browser/PWA status bar color
  var metaTheme = document.getElementById('metaThemeColor');
  if (metaTheme) metaTheme.setAttribute('content', mode === 'light' ? '#f4f6fb' : '#0f0f13');
}
...
function scsToggleTheme() {
  var current = localStorage.getItem('app-theme') || 'dark';
  var next = current === 'light' ? 'dark' : 'light';
  // Set pending theme and call the exact same apply function Settings uses
  _appearPending.theme = next;
  appearApply();
  // Sync topbar button icons
