...
async function dbGetPlayers(forceFresh = false) {
  ...
  try {
    let normalized;
    if (club.id) {
      // New schema: players linked via memberships table
...
async function dbOverrideRating(playerId, newRating) {
  ...
  // Update club_rating in memberships
  const club2 = getMyClub();
  if (club2.id) {
    await sbPatch('memberships', `player_id=eq.${playerId}&club_id=eq.${club2.id}`, { club_rating: rounded });
  }
  localStorage.removeItem(CACHE_PLAYERS);
  localStorage.removeItem(CACHE_TIMESTAMP);
  ...
  try {
    ...
    const upserts = players
      ...
      .map(p => {
        ...
        return Promise.resolve(); // live_sessions replaced by sessions.rounds_data
      });
  ...
  try {
    ...
    const writes = rows
      ...
      try {
        ...
        const playerRows = _mrows.length
          ? await sbGet('players', `id=eq.${_mrows[0].player_id}&select=id,sessions`)
          : [];
        if (playerRows && playerRows.length) {
          ...
          await sbPatch('players', `id=eq.${playerRows[0].id}`, { sessions: updated });
...
async function syncGlobalPlayersCache() {
  try {
    const club = getMyClub();
    if (!club.id) return;
    const raw = await sbGet('memberships', `club_id=eq.${club.id}&order=nickname.asc&select=nickname,club_rating,players(gender,global_rating)`);
...
async function dbDeleteClub(clubId) {
  // Cascade deletes memberships, sessions, join_requests
...
async function dbClaimSessionSlots(playerNames) {
  ...
  for (const name of playerNames) {
    try {
      await sbPatch('memberships', `club_id=eq.${club.id}&nickname=ilike.${encodeURIComponent(name)}`, {
        is_playing: true
      });
    } catch(e) { /* silent per player */ }
  }
...
async function dbReleaseMySession() {
  try {
    const _rmc = getMyClub();
    if (_rmc.id) await sbPatch('memberships', `club_id=eq.${_rmc.id}&is_playing=eq.true`, { is_playing: false });
  } catch(e) { /* silent */ }
}
...
async function dbGetUnavailablePlayers() {
  try {
    ...
    const _uclub = getMyClub();
    if (!_uclub.id) return new Set();
    const rows = await sbGet('memberships',
      `club_id=eq.${_uclub.id}&is_playing=eq.true&select=nickname`
    );
