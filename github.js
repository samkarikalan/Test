...
async function dbAddPlayer(name, gender, homeClub) {
  ...
  if (exists) throw new Error(`Player "${name}" already exists in this club.`);
...
/// ============================================================
/// SESSION SYNC — called silently at end of session
/// Matches session players to GitHub DB by name + homeClub
/// Updates ratings for matched players only
/// No UI interaction — errors logged to console only
/// ============================================================

async function githubSyncSessionRatings() {
  try {
    // Get all players from GitHub (use cache if available)
    const dbPlayers = await dbGetPlayers();
    if (!dbPlayers || dbPlayers.length === 0) return;

    // Build updates array — only players that exist in GitHub DB
    const updates = [];
    for (const sp of schedulerState.allPlayers) {
      const name      = sp.name;
      const localRating = getRating(name);

      // Match by name (case-insensitive) — any club
    ...
    await dbSyncRatings(updates);
    console.log(`✅ GitHub sync: ${updates.length} player(s) updated`);

  } catch (e) {
    // Silent fail — local ratings are still correct
    console.warn("GitHub sync failed (will retry next session):", e.message);
  }
}
