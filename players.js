...
function deletePlayer(i) {
  ...
  schedulerState.activeplayers.splice(0, schedulerState.activeplayers.length,
    ...schedulerState.allPlayers.filter(p => p.active).map(p => p.name).reverse());
  // Release is_playing in DB — same as playerPlayingRelease() in settings.js
  try {
    const _rc = (typeof getMyClub === 'function') ? getMyClub() : { id: null };
    if (_rc && _rc.id) {
      sbPatch('memberships', `club_id=eq.${_rc.id}&nickname=ilike.${deletedPlayer}`, { is_playing: false });
    }
  } catch(e) { console.warn('Could not release is_playing for ' + deletedPlayer, e); }
  updatePlayerList();
  updateFixedPairSelectors();
  refreshFixedCards();
}
...
function addPlayersFromInputUI(replace) {
  ...
  // Replace clears existing list first — release old players from DB before wiping
  if (replace) {
    const oldNames = schedulerState.allPlayers.filter(p => p.active).map(p => p.name);
    if (oldNames.length > 0 && typeof dbReleaseSessionSlots === 'function') {
      dbReleaseSessionSlots(oldNames);
    }
...
function createPlayerCard(player, index) {
  ...
  // Touch drag for mobile — non-passive so we can preventDefault once drag is confirmed
  card.addEventListener("touchstart", onTouchDragStart, { passive: false });
  card.addEventListener("touchmove",  onTouchDragMove,  { passive: false });
  card.addEventListener("touchend",   onTouchDragEnd,   { passive: true });
  card.addEventListener("touchcancel",onTouchDragCancel,{ passive: true });
...
/* ── Touch drag-to-reorder ──
   Uses a 200 ms long-press threshold so a quick vertical swipe
   scrolls the page normally; only a deliberate hold activates drag.
── */
let _touchDragEl    = null;
let _touchDragIdx   = null;
let _touchStartY    = 0;
let _touchStartX    = 0;
let _dragActivated  = false;   // true once long-press threshold passed
let _longPressTimer = null;

const LONG_PRESS_MS  = 200;   // ms hold before drag activates
const DRAG_THRESHOLD = 6;     // px movement that cancels long-press
...
function _activateDrag() {
  if (!_touchDragEl || _dragActivated) return;
  _dragActivated = true;
  _touchDragEl.style.opacity   = '0.5';
  _touchDragEl.style.transform = 'scale(1.02)';
  _touchDragEl.style.zIndex    = '100';
  _touchDragEl.style.transition = 'none';
  // Haptic if supported
  if (navigator.vibrate) navigator.vibrate(30);
}

function onTouchDragStart(e) {
  // Don't hijack taps on interactive children (checkbox, button)
  ...
  // Start long-press timer — do NOT preventDefault yet, so scroll still works
  _longPressTimer = setTimeout(_activateDrag, LONG_PRESS_MS);
}
...
function onTouchDragMove(e) {
  ...
  if (!_dragActivated) {
    // If user moved more than threshold before long-press fired → cancel drag, allow scroll
    if (dx > DRAG_THRESHOLD || dy > DRAG_THRESHOLD) {
      _cancelLongPress();
      _touchDragEl = null;
    }
    return; // don't preventDefault — let browser scroll
  }
  ...
  const offsetDy = y - _touchStartY;
  _touchDragEl.style.transform = `translateY(${offsetDy}px) scale(1.02)`;
...
function onTouchDragCancel() {
  ...
  _touchDragEl.style.transition = '';
  const container = document.getElementById('playerList');
  if (container) container.querySelectorAll('.player-edit-card').forEach(c => { c.style.boxShadow = ''; });
  _touchDragEl   = null;
  _touchDragIdx  = null;
  _dragActivated = false;
}
